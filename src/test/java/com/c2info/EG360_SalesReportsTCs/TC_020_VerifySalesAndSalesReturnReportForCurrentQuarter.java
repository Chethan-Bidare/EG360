...
public class TC_020_VerifySalesAndSalesReturnReportForCurrentQuarter extends TestBase{
	...
	public void verifyBranchName() throws InterruptedException, SQLException{
		...
		for(String str : set){
			...
			String query = "select act_mst.c_name,inv_mst.c_br_code from inv_mst, act_mst "
					+ "where inv_mst.c_br_code=act_mst.c_code and  c_br_code='"+brCode+"'";
	...
	public void verifyBranchShortName() throws InterruptedException, SQLException{
		...
		for(String str : set){
			...
			String query = "select act_mst.c_sh_name,inv_mst.c_br_code from inv_mst, act_mst"
					+ " where inv_mst.c_br_code=act_mst.c_code and  c_br_code='"+brCode+"'";
			ResultSet val = db.getData(query);
			String actualValue = null ;
			while(val.next()){
	...
	public void verifyBranchWiseNumberOfInvoices() throws InterruptedException, SQLException{
		...
		for(String str : set){
			...
			String query = "select count(*) from inv_mst where c_br_code='"+brCode+"' and "
					+ "n_cancel_flag=0 and d_date>='"+getCurrentQuarterStartDate()+"' and d_date<='"+getCurrentQuarterEndDate()+"'";
			//d_date>='"+getCurrentQuarterStartDate()+"' and d_date<='"+getCurrentQuarterEndDate()+"'
	...
	public void verifyBranchwiseNumberOfCustomers() throws InterruptedException, SQLException{
		...
		for(String str : set){
			...
			String query = "select sum(t.invCust)-sum(t.crntCust) from "
					+ "(SELECT sum(distinct c_cust_code) as invCust,0 as crntCust "
					+ "FROM inv_mst where d_date>='"+getCurrentQuarterStartDate()+"' and "
							+ "d_date<='"+getCurrentQuarterEndDate()+"' and c_br_code='"+brCode+"'  and n_cancel_flag=0 "
							+ "union "
							+ "SELECT 0 as invCust,sum(distinct c_cust_code) as crntCust "
							+ "FROM crnt_mst where d_date>='"+getCurrentQuarterStartDate()+"' and "
									+ "d_date<='"+getCurrentQuarterEndDate()+"' and c_br_code='"+brCode+"' and n_cancel_flag=0)t";
			ResultSet val = db.getData(query);
			String actualValue = null ;
			while(val.next()){
				actualValue = val.getString(1);
	...
	public void verifyBranchwiseDiscAmount() throws InterruptedException, SQLException{
		...
		for(String str : set){
			...
			String query = "select sum(t.invDisc)-sum(t.crntDisc) from "
					+ "(SELECT sum(n_discount) as invDisc,0 as crntDisc "
					+ "FROM inv_mst where d_date>='"+getCurrentQuarterStartDate()+"' and "
							+ "d_date<='"+getCurrentQuarterEndDate()+"' and c_br_code='"+brCode+"' and n_cancel_flag=0 "
					+ "union "
					+ "SELECT 0 as invDisc,sum(n_discount) as crntDisc "
					+ "FROM crnt_mst where d_date>='"+getCurrentQuarterStartDate()+"' and "
							+ "d_date<='"+getCurrentQuarterEndDate()+"' and c_br_code='"+brCode+"' and n_cancel_flag=0)t";
			ResultSet val = db.getData(query);
	...
	public void verifyBranchwiseInvoiceValAfterDisc() throws InterruptedException, SQLException{
		...
		for(String str : set){
			...
			String query = "select sum(t.invVal)-sum(t.crntVal) from "
					+ "(SELECT sum(n_taxable_amt) as invVal,0 as crntVal "
					+ "FROM inv_mst where d_date>='"+getCurrentQuarterStartDate()+"' and "
							+ "d_date<='"+getCurrentQuarterEndDate()+"'  and c_br_code='"+brCode+"' n_cancel_flag=0 "
					+ "union "
					+ "SELECT 0 as invVal,sum(n_taxable_amt) as crntVal "
					+ "FROM crnt_mst where d_date>='"+getCurrentQuarterStartDate()+"' and "
							+ "d_date<='"+getCurrentQuarterEndDate()+"'  and c_br_code='"+brCode+"' n_cancel_flag=0)t";
			ResultSet val = db.getData(query);
			double actualValue = 0;
			while(val.next()){
				actualValue = val.getDouble(1);
	...
	public void verifyBranchwiseInvoiceValTax() throws InterruptedException, SQLException{
		...
		for(String str : set){
			...
			double expectedValue = Double.parseDouble(expectValue);
			String query ="select sum(t.invVal)-sum(t.crntVal) from "
					+ "(SELECT sum(n_total) as invVal,0 as crntVal "
					+ "FROM inv_mst where d_date>='"+getCurrentQuarterStartDate()+"' and "
							+ "d_date<='"+getCurrentQuarterEndDate()+"'  and c_br_code='"+brCode+"' n_cancel_flag=0 "
					+ "union "
					+ "SELECT 0 as invVal,sum(n_total) as crntVal FROM crnt_mst "
					+ "where d_date>='"+getCurrentQuarterStartDate()+"' and "
							+ "d_date<='"+getCurrentQuarterEndDate()+"'  and c_br_code='"+brCode+"' n_cancel_flag=0)t";
			ResultSet val = db.getData(query);
			double actualValue = 0;
			while(val.next()){
	...
	public void verifyBranchwiseTaxAmount() throws InterruptedException, SQLException{
		...
		for(String str : set){
			...
			String query = "select sum(t.invTax)-sum(t.crntTax) from "
					+ "(SELECT sum(n_cgst_amt + n_sgst_amt + n_igst_amt + n_cess_amt) as invTax,0 as crntTax "
					+ "FROM inv_mst where d_date>='"+getCurrentQuarterStartDate()+"' and "
							+ "d_date<='"+getCurrentQuarterEndDate()+"'  and c_br_code='"+brCode+"' n_cancel_flag=0 "
					+ "union "
					+ "SELECT 0 as invTax,sum(n_cgst_amt + n_sgst_amt + n_igst_amt + n_cess_amt) as crntTax "
					+ "FROM crnt_mst where d_date>='"+getCurrentQuarterStartDate()+"' and "
							+ "d_date<='"+getCurrentQuarterEndDate()+"'  and c_br_code='"+brCode+"' n_cancel_flag=0)t";
			ResultSet val = db.getData(query);
			double actualValue = 0;
	...
	public void verifyBranchwiseCGSTAmount() throws InterruptedException, SQLException{
		...
		for(String str : set){
			...
			double expectedValue = Double.parseDouble(expectValue);
			String query = "select sum(t.invTax)-sum(t.crntTax) from "
					+ "(SELECT sum(n_cgst_amt) as invTax,0 as crntTax "
					+ "FROM inv_mst where d_date>='"+getCurrentQuarterStartDate()+"' and "
							+ "d_date<='"+getCurrentQuarterEndDate()+"'  and c_br_code='"+brCode+"' n_cancel_flag=0 "
					+ "union "
					+ "SELECT 0 as invTax,sum(n_cgst_amt) as crntTax "
					+ "FROM crnt_mst where d_date>='"+getCurrentQuarterStartDate()+"' and "
							+ "d_date<='"+getCurrentQuarterEndDate()+"' and c_br_code='"+brCode+"' n_cancel_flag=0)t";
			ResultSet val = db.getData(query);
			double actualValue = 0;
	...
	public void verifyBranchwiseSGSTAmount() throws InterruptedException, SQLException{
		...
		for(String str : set){
			...
			double expectedValue = Double.parseDouble(expectValue);
			String query = "select sum(t.invTax)-sum(t.crntTax) from "
					+ "(SELECT sum(n_sgst_amt) as invTax,0 as crntTax "
					+ "FROM inv_mst where d_date>='"+getCurrentQuarterStartDate()+"' and "
							+ "d_date<='"+getCurrentQuarterEndDate()+"' and c_br_code='"+brCode+"' and n_cancel_flag=0 "
					+ "union "
					+ "SELECT 0 as invTax,sum(n_sgst_amt) as crntTax "
					+ "FROM crnt_mst where d_date>='"+getCurrentQuarterStartDate()+"' and "
							+ "d_date<='"+getCurrentQuarterEndDate()+"' and c_br_code='"+brCode+"' and n_cancel_flag=0)t";
			ResultSet val = db.getData(query);
			double actualValue = 0;
	...
	public void verifyBranchwiseIGSTAmount() throws InterruptedException, SQLException{
		...
		for(String str : set){
			...
			double expectedValue = Double.parseDouble(expectValue);
			String query = "select sum(t.invTax)-sum(t.crntTax) from "
					+ "(SELECT sum(n_igst_amt) as invTax,0 as crntTax "
					+ "FROM inv_mst where d_date>='"+getCurrentQuarterStartDate()+"' and "
							+ "d_date<='"+getCurrentQuarterEndDate()+"'  and c_br_code='"+brCode+"' and n_cancel_flag=0 "
					+ "union "
					+ "SELECT 0 as invTax,sum(n_igst_amt) as crntTax "
					+ "FROM crnt_mst where d_date>='"+getCurrentQuarterStartDate()+"' and "
							+ "d_date<='"+getCurrentQuarterEndDate()+"'  and c_br_code='"+brCode+"' and n_cancel_flag=0)t";
			ResultSet val = db.getData(query);
			double actualValue = 0;
	...
	public void verifyBranchwiseCessAmount() throws InterruptedException, SQLException{
		...
		for(String str : set){
			...
			double expectedValue = Double.parseDouble(expectValue);
			String query = "select sum(t.invTax)-sum(t.crntTax) from "
					+ "(SELECT sum(n_cess_amt) as invTax,0 as crntTax "
					+ "FROM inv_mst where d_date>='"+getCurrentQuarterStartDate()+"' and d_date<='"+getCurrentQuarterEndDate()+"' "
							+ " and c_br_code='"+brCode+"' and n_cancel_flag=0 "
					+ "union "
					+ "SELECT 0 as invTax,sum(n_cess_amt) as crntTax "
					+ "FROM crnt_mst where d_date>='"+getCurrentQuarterStartDate()+"'"
							+ " and d_date<='"+getCurrentQuarterEndDate()+"'  and c_br_code='"+brCode+"' and n_cancel_flag=0)t";
			ResultSet val = db.getData(query);
			double actualValue = 0;
			while(val.next()){
				actualValue = val.getDouble(1);
	...
	public void verifyBranchwiseServiceChargeAmount() throws InterruptedException, SQLException{
		...
		for(String str : set){
			...
			double expectedValue = Double.parseDouble(expectValue);
			String query = "select sum(t.invTax)-sum(t.crntTax) from "
					+ "(SELECT sum(n_service_chg) as invTax,0 as crntTax "
					+ "   FROM inv_mst where d_date>='"+getCurrentQuarterStartDate()+"' and "
							+ "d_date<='"+getCurrentQuarterEndDate()+"'  and c_br_code='"+brCode+"' and n_cancel_flag=0 "
					+ "union "
					+ "SELECT 0 as invTax,sum(n_service_chg) as crntTax "
					+     "FROM crnt_mst where d_date>='"+getCurrentQuarterStartDate()+"' and "
							+ "d_date<='"+getCurrentQuarterEndDate()+"'  and c_br_code='"+brCode+"' and n_cancel_flag=0)t";
			ResultSet val = db.getData(query);
			double actualValue = 0;
