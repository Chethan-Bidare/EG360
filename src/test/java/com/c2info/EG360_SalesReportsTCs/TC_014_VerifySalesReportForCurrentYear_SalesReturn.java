...
public class TC_014_VerifySalesReportForCurrentYear_SalesReturn extends TestBase{
	...
	public void verifyBranchwiseInvoiceValAfterDisc() throws InterruptedException, SQLException{
		...
		for(String str : set){
			...
			expectValue = expectValue.replaceAll("-","").trim();
			double expectedValue = Double.parseDouble(expectValue);
			String query = "select sum(n_taxable_amt) from crnt_mst where c_br_code='"+brCode+"' and  d_date>='"+getCurrentYearStartDate()+"' and d_date<='"+getCurrentYearEndDate()+"' and n_cancel_flag=0";
			ResultSet val = db.getData(query);
			double actualValue = 0;
			while(val.next()){
				actualValue = val.getDouble(1);
